...
parser = argparse.ArgumentParser(description="A program to spoof commits to GitHub so that potential employers won't see that you're lazy and don't actually code lol. Only tested on Windows idk if it works on Linux or anything else")
...
parser.add_argument("--blatant", default="false", nargs=1, metavar="boolean", type=str, help="Commit every single day")
...
blatant = args.blatant[0].lower() == "true"
...
try:
    ...
    start_date = datetime.date(int(git_start_date[0 : 4]), int(git_start_date[5 : 7]), int(git_start_date[8 : 10]))
    end_date = datetime.date(int(git_end_date[0 : 4]), int(git_end_date[5 : 7]), int(git_end_date[8 : 10]))
except:
...
if not os.path.exists(args.repo_path[0]):
    ...
    for i in range(num_days):
        if ( random.choice(should_commit) or blatant ):
